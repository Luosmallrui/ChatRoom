...
func (s *ContactApplyService) Create(ctx context.Context, opt *ContactApplyCreateOpt) error {
	...
	_ = s.PushMessage.Push(ctx, types.ImTopicChat, &types.SubscribeMessage{
		Event: types.SubEventContactApply,
		Payload: jsonutil.Encode(types.SubEventContactApplyPayload{
			ApplyId: apply.Id,
			Type:    1,
		}),
	})
